 ...
 *
 * A snapshot records the total USD-equivalent value of the portfolio
 * on a given date. One snapshot per day is taken automatically by the
 * daily cron job; it can also be triggered manually.
 *
 * HKD/non-USD assets: we use a fixed approximate rate (1 USD = 7.8 HKD)
...
const HKD_TO_USD = 1 / 7.8;

function toUsd(value: number, currency: string): number {
  if (currency === "USD") return value;
  if (currency === "HKD") return value * HKD_TO_USD;
...
export async function takeSnapshot(dateOverride?: string): Promise<{
  ...
  const totalValue = updated.reduce(
    (s, a) => s + toUsd(a.quantity * a.currentPrice, a.currency), 0
  );
  const totalCost = updated.reduce(
    (s, a) => s + toUsd(a.quantity * a.purchasePrice, a.currency), 0
  );
